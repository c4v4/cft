...
class ThreePhase {
public:
    ThreePhase(SubInstance& subinst_, cft::prng_t& rnd_)
        : subinst(subinst_)
        , subgradient(subinst)
        , greedy(subinst)
        , col_fixing(subinst, greedy)
        , rnd(rnd_) {
    }
    ...
    std::pair<GlobalSolution, GlobalMultipliers> operator()(const real_t global_UB) {
        ...
        do {
            ...
            auto const subgrad_UB = glo_UB_star - subinst.get_fixed_cost();

            // 1. SUBGRADIENT PHASE
            u_k = subgradient.solve(
                subgrad_UB,
                u_k);  // use S_curr_cost because u_k refers to the current sub-problem
            ...
            for (LocalMultipliers& u : u_list) {
                ...
                if (S_cost < S_curr_cost) {
                    ...
                    if (gS_cost < glo_UB_star) {
                        ...
                        IF_VERBOSE {
                            fmt::print("│ ──> [{:3}] Improved local UB: {} (global value {}, best "
                                       "is {})\n",
                                       &u - &u_list[0],
                                       S_cost,
                                       GlobalSolution(subinst, S).get_cost(),
                                       glo_UB_star);
                        }
                    }
                }
            ...
            IF_DEBUG {
                CountSet cov;
                cov.reset_covered(subinst.get_instance().get_cols(),
                                  glo_S_curr,
                                  subinst.get_instance().get_nrows());
                assert(cov.get_uncovered() == 0);
            }
