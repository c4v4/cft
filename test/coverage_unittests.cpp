...
TEST_CASE("test_single_row_cover_set_coverage") {
    ...
    auto cs = make_cover_counters(nrows);
...
TEST_CASE("test_single_row_bit_set_coverage") {
...
TEST_CASE("test_multiples_rows_bit_set_coverage") {
    ...
    for (cidx_t j = 0; j < 4; ++j)
        REQUIRE(bs.cover(cols[j]) == cols[j].size());
    for (cidx_t j = 4; j < cols.size(); ++j) {
        REQUIRE(bs.cover(cols[j]) == 0);
        REQUIRE(bs.uncover(cols[j]) == cols[j].size());
        REQUIRE(bs.uncover(cols[j]) == 0);
    }
