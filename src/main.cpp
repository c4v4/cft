...
int main(int argc, char const** argv) {
    ...
    while (!inst.rows.empty()) {
        ...
        auto lagr_mult = cft::compute_greedy_multipliers(core_inst);
        auto sol       = cft::Solution();
        greedy(core_inst, lagr_mult, sol);

        auto cutoff = std::min(sol.cost, best_sol.cost - fixing.fixed_cost);
        auto opt_lb = cft::optimize(inst, core_inst, cutoff, sol.cost, lagr_mult);
        auto exp_lb = cft::explore(core_inst, greedy, cutoff, sol, lagr_mult);
        ...
        if (sol.cost + fixing.fixed_cost < best_sol.cost) {
            ...
            IF_DEBUG(check_solution(core_inst, sol));
